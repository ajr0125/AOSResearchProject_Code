function [pd] = OSGO_CFAR(numTrials, PFA, N, magSqGaussNoise, signalData, k)
...
%where the lagging window sum is greater than the leading window sum
osgoNME = leadWindow(k,:);
idx = lagSum>leadSum;
osgoNME(idx) = lagWindow(k,idx);

pd = sum(signalData > T * osgoNME)/numTrials;